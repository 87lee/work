...
var myData = {};
$(function () {
    AjaxGet('/module/lists', function(data){
...
function delTableInfo(){
    if(myData.id){
        if( confirm('确定删除？') ){
            AjaxGet('/module/delete?id=' + myData.id, function(){
                AjaxGet('/module/lists', function(data){
                    createElem(data.modules);
                    myData.id = null;
                });
...
function delTableInfo2(){
    if(myData.id2){
        if( confirm('确定删除？') ){
            AjaxGet('/module/delete?id=' + myData.parentId + '&sub_id=' + myData.id2, function(){
                AjaxGet('/module/lists', function(data){
                    var sub_modules = getSubModules(data.modules);
                    createElem2(sub_modules);
                    myData.id2 = null;
...
$('#subModule').click(function() {
    ...
    if(title == '新增'){
        ...
        AjaxPost('/module/add', data, function(){
            $('#myModal').modal('hide');
            AjaxGet('/module/lists', function(data){
                createElem(data.modules);
            });
        ...
        AjaxPost('/module/sub/add', data, function(){
            $('#myModal').modal('hide');
            AjaxGet('/module/lists', function(data){
                var sub_modules = getSubModules(data.modules);
                createElem2(sub_modules);

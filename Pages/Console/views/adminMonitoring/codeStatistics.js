//@ sourceURL=adminMonitoring.codeStatistics.js
var myData = {};
...
$(function () {
    ...
    AjaxGet('/Monitoring/getInterface', function(data){
        selectPorject(data);
        $('#page-content').show();
    });
...
$('#getChartData').on('click', function(){
    ...
    AjaxGet('/Monitoring/getAdmin?startTime='+ (0 | (startTime / 60000)) * 60 +'&endTime='+ (0 | (endTime / 60000)) * 60 +'&interval='+ interval +'&interface='+ project +'&codeType=' + codeType, function(data){
...
function selectPorject(data){
    ...
    for( var i=0; i<arr.length; i++ ){
        con += '<option value="'+arr[i]+'">'+arr[i]+'</option>';
    }
    $select.html(con);
}

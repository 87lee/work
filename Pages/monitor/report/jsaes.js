...
function ase_block_xor(block, iv) {
	for (var i = 0; i < 16; i++) {
		block[i] ^= iv[i];
	}
	return block;
...
function aes_block_cpy(dst, src, dst_off, src_off) {
	for (var i = 0; i < 16; i++) {
		dst[dst_off + i] = src[src_off + i];
	}
}

function aes_tail(data) {
	var c = data[data.length - 1];
	if (!c || c > 0xf) {
		return 0;
	}
	for (var i = data.length - c; i < data.length - 1; ++i) {
		if (data[i] != c) {
			return 0;
		}
	}
...
function aes_cbc_process(data, key, pre, func, post) {
	var out = new Array((data.length + 15) & 0xFFFFFFF0);
	var block = new Array(16);
	AES_Init();
	AES_ExpandKey(key);
	for (var j = 0; j < data.length; j = j + 16) {
		aes_block_cpy(block, data, 0, j);
		if (pre) {
			ase_block_xor(block, pre);
		}
		func(block, key);
		if (pre) {
			aes_block_cpy(pre, block, 0, 0);
		}
		if (post) {
			ase_block_xor(block, post);
			aes_block_cpy(post, data, 0, j);
		}
		aes_block_cpy(out, block, j, 0);
	}
	return out;
}

function aes_cbc_encrypt(data, key, iv) {
	return aes_cbc_process(data, key, iv, AES_Encrypt, null);
}
function aes_cbc_decrypt(data, key, iv) {
	return aes_cbc_process(data, key, null, AES_Decrypt, iv);
}

function aes_encrypt(data, key) {
	return aes_cbc_process(data, key, null, AES_Encrypt, null);
}
function aes_decrypt(data, key) {
	return aes_cbc_process(data, key, null, AES_Decrypt, null);
}
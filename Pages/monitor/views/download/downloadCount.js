//@ sourceURL=download.downloadCount.js
var myData = {};
...
$('#getChartData').on('click', function(){
    ...
    AjaxGet('/Monitoring/home/Monitoring/getDownloadHistory?startTime='+ (0 | (startTime / 60000)) * 60 +'&endTime='+ (0 | (endTime / 60000)) * 60 +'&interval='+ interval +'&url='+ project +'&codeType=' + codeType, function(data){
...
function selectPorject(data){
    ...
    for( var i=0; i<arr.length; i++ ){
        con += '<option value="'+arr[i].url+'">'+arr[i].name+'</option>';
    }
    // $select.html(con);

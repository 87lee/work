/*全局变量*/
var hostUrl='http://192.168.1.199:180/Customer/Public/';
var myConfig = {
...
function listenMyPage(table, nPage, fn, tmp, otable) {
    ...
    $('#page-content').on('click', '#'+ table +'_paginate ul li a', function() {
        ...
        if (val === '上一页' && !$(this).parent().hasClass('disabled')) {
            currentPage--;
            if (table === 'platformListTable') {
                order.pform = filter;
                console.log(order);
                fn(currentPage, order, table);
            }else if (table === 'vendorIdListTable') {
                order.venid = filter;
                fn(currentPage, order, table);
            }else{
            ...
            if (table === 'platformListTable') {
                fn(currentPage, {"pform": filter}, table);
            }else if (table === 'vendorIdListTable') {

                fn(currentPage, {"venid": filter}, table);
            }else{
                fn(currentPage, filter, table);
            }
        }

				...
				if (!empty($arr)) {
					...
					if (isAdmin()) {
						if ($res = $this->where("id IN (".$sqlId.")")->select()) {
						    ...
						    //判断应用是否有关联项目，有关联项目不允许删除
						    $projects=M('App')->alias('a')->join('INNER JOIN tb_project p ON a.id=p.app_id')->where(array('p.is_del'=>0,'a.id'=>array('IN',$arr)))->find();
						    if(!empty($projects)){
						        result('请先删除应用的关联项目。');
						    }else{

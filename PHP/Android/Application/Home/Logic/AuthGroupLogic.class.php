...
class AuthGroupLogic extends Model
{
    ...
    {
        $gid = session('androidIsLogin.gids');
        if (empty($gid)) {
            throw new \LogicException('非法登录', C('FORBIDDEN'));
        } else {
            $subGroupId = M('AuthGroup')->where(array('id' => array('IN', $gid)))->getField('sub_group', true);
            if (empty($subGroupId)) {
                return array('list' => array());
            } else {
                $ids = array();
                foreach ($subGroupId as $v) {
                    $ids = array_merge($ids, explode(',', trim($v, ',')));
                }

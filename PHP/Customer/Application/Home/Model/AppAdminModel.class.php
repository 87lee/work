<?php
	...
	class AppAdminModel extends \Think\Model
	{
		...
		{
			if (!empty($put['app'])&&!empty($put['name'])&&!empty($put['operator'])&&($put['operation']=='test' || $put['operation']=='publish' )) {
			...
			if ( $user = isAdmin() ) {
				...
				if (!empty($get['id'])) {
					...
					if (!empty($get['name'])) {
						...
						$where = " `name` like '%".$get['name']."%' or `app` like '%".$get['name']."%' or `operator` IN (".$UserName.") ";
					}
				...
				if (!empty($get['id'])) {
					...
					if (!empty($get['name'])){
						$UserName = D('User')->getAllNameForUserName($get['name']);
						$where .= " and ( `name` like '%".$get['name']."%' or `app` like '%".$get['name']."%' or `operator` IN (".$UserName.") )";
					}
					if (!empty($get['page'])&&!empty($get['pageSize'])) {
						$get['page'] = $get['page']*$get['pageSize'] - $get['pageSize'];
						$res['extra'] = $this->limit($get['page'],$get['pageSize'])->where($where)->select();

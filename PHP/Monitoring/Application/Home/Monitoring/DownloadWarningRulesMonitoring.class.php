<?php
	...
	class DownloadWarningRulesMonitoring extends \Think\Model
	{
		protected $tableName = 'download_warning_rules';
		...
		public function addWarningRules($put)
		{
			...
			if (!D('Domain','Monitoring')->getOneInId((int)$put['projectId'])) {
				result('该接口不存在');
			}
			if ($put['compare'] == '&gt;'  /*>*/) {
				$put['compare'] = '>';
		...
		public function warningRulesLists($get)
		{
			...
			if (empty($get['downloadId'])) {
				return $res = array(
					"extra"=>array(),
					'count'=>0
					);
			}
			$where = 'project_id ='.(int)$get['downloadId'] . " and invalid = 0 ";

			$field = "a.id,a.period,a.method,a.code,a.compare,a.status,a.value,b.rules_id,b.group_id,c.name";


			$join = " LEFT JOIN tb_download_warning_rules_object AS b on a.id = b.rules_id  ";
			$join .= " LEFT JOIN tb_warning_group AS c on c.id = b.group_id ";

			/*if (!empty($get['url'])) {
				$field = ',d.url';
				$join .= " LEFT JOIN tb_domain AS d on d.id = a.project_id ";
			}*/
		...
		public function getAllWarningRulesLists()
		{


			// $where = 'project_id ='.(int)$get['downloadId'];
			$where = ' invalid = 0 ';
			$field = "a.id,a.period,a.method,a.code,a.compare,a.status,a.value,b.rules_id,b.group_id,c.name,d.url as rulesWhere,d.name as rulesName,f.id as uid,f.email";
		...
		public function getWarningRules($get)
		{
			...
			$field = "a.id,a.period,a.method,a.code,a.compare,a.status,a.value,b.name,b.url ";
			$join = " LEFT JOIN tb_domain AS b on b.id = a.project_id ";
			if (!empty($get['name'])) {
				$where .= " and (b.name LIKE '%" .$get['name']. "%' OR b.url LIKE '%" .$get['name']. "%')";
			}
			if (!empty($get['page'])&&!empty($get['pageSize'])) {

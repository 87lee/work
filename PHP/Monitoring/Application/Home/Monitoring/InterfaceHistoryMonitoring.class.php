		...
		public function getCodeTypeHistory($get)
		{
			...
			*/
				...
				if (!empty($get['interface'])&&!empty($get['codeType']) && ($get['codeType'] !='all') ) {
					$where .= "' AND interface = '" .$get['interface']."' AND codetype >= ".$get['codeType']."00 AND codetype <=".$get['codeType']."99";
					...
					$where .= "'  AND codetype >= ".$get['codeType']."00 AND codetype <=".$get['codeType']."99";
					$codeType = $this->where(" codetype >= ".$get['codeType']."00 AND codetype <=".$get['codeType']."99")->field('codetype')->group('codetype')->select();

				}elseif (!empty($get['interface'])) {
					$where .= "' AND interface = '" .$get['interface']."'" ;
		...
		public function getArrForCheckRules($get)
		{
			if (!empty($get['endTime'])&&!empty($get['startTime'])&&!empty($get['startCode'])&&!empty($get['endCode'])) {
				...
				if (!empty($get['rulesWhere'])) {
					$where .= " AND interface = '".$get['rulesWhere']."' ";
				}
				...
				if (!empty($get['method']) && $get['method'] == 'percent') {
					$field = "sum(codenum) as num ,codetype,interface";
					$group = "codetype,interface";
					$res = $this->field($field)->where($where)->group($group)->select();
				}
				$field = "sum(codenum) as num ,concat('".$startCode.'-'.$endCode."') as codetype,interface";
				$group = "interface";

<?php
	...
	class InterfaceWarningRulesHistoryMonitoring extends \Think\Model
	{
		protected $tableName = 'interface_warning_rules_history';
		...
		public function getWarningRulesHistory()
		{
			...
			// $where = 'a.status =1';
			$join  = ' LEFT JOIN tb_interface_warning_rules AS b ON a.rules_id = b.id  LEFT JOIN  tb_interface_group_item AS c ON b.project_id = c.id';
			$order = ' a.id asc';
			$field = 'a.id,a.status,a.rules_id,a.time,b.code,b.period,b.method,b.compare,b.value,c.name,c.interface';
			$group = 'a.rules_id';
			...
			$sql = $this->field($field)->table(" (SELECT * FROM  tb_interface_warning_rules_history ORDER BY id DESC) ")->alias(" AS a")->where($where)->join($join)->order($order)->group($group)->select(false);
			...
			$count = $Model->query( " SELECT count(*) as count FROM (". $sql . ") AS d WHERE d.status = 1 LIMIT 1");

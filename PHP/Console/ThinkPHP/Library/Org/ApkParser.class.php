...
//----------------------
  ...
  public function getLinkinChannel() {
    for ($id=0; true; $id++) {
	$act = $this->getAttribute ("manifest/application/meta-data[{$id}]", 'android:name');
        if (!$act) break;
	if ($act == 'LINKIN_CHANNEL') {
		return $this->getAttribute ("manifest/application/meta-data[{$id}]", 'android:value');
	}
    }
    return NULL;
  }

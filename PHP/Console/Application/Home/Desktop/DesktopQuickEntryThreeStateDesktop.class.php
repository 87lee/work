<?php
	...
	class DesktopQuickEntryThreeStateDesktop extends \Think\Model
	{
		protected $tableName = 'desktop_quick_entry_three_state';
				...
				if ($desktopJson) {
					...
					if (!empty($quickInfo)) {
						foreach ($quickInfo as $key => $value) {
							if ( isset($value['index'])&&!empty($value['focusedDrawableC'])&&!empty($value['focusedDrawableB'])&&!empty($value['focusedDrawableA'])&&!empty($value['drawableC'])&&!empty($value['drawableB'])&&!empty($value['drawableA'])&&!empty($value['eventC'])&&!empty($value['eventB'])&&!empty($value['eventA'])&&!empty($value['eventType'])&&!empty($value['name'])&&isset($value['x'])&&isset($value['y'])) {
							...
							$desktopJson['items'][$key] = array(
								...
								'eventB'=>$value['eventB'],
								'eventC'=>$value['eventC'],
								'drawableA'=>$value['drawableA'],
								'drawableB'=>$value['drawableB'],
								'drawableC'=>$value['drawableC'],
								'focusedDrawableA'=>$value['focusedDrawableA'],
								'focusedDrawableB'=>$value['focusedDrawableB'],
								'focusedDrawableC'=>$value['focusedDrawableC'],
							);
							...
							$actionData = desktopWidgetCheckAction($value,$error,'三态快捷入口');
							...
							/*switch ($value['type']) {
								case 'APP':
									if (isset($value['pkgName'])) {
										...
										if ($error) {
											return $result = array('reason'=>'三态快捷入口id为'.$value['index'].'action数据出错');
											// return false;
										}else{

											result('三态快捷入口id为'.$value['index'].'action数据出错');
										}
									}
									break;
								case 'ACTION':
							...
							if ($error) {
								return $result = array('reason'=>'三态快捷入口参数出错');
								// return false;
							}else{
								result('三态快捷入口参数出错');
							}
						...
						if ($error) {
							return $result = array('reason'=>'三态快捷入口action数据为空');
							// return false;
						}else{
							result('三态快捷入口action数据为空');
						}
					}

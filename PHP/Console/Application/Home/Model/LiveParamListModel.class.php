<?php 
	...
	use Think\Model;
	class LiveParamListModel extends Model 
	{
		...
		/*public function __construct()  
    		...
    		{
    			$res = M('LiveParamType')->find($typeId);
    			if ($res) {
    		...
    		public function paramLists($typeId = ' ')
    		{
				...
				$type['extra'] = D('LiveParamType')->select();
				// $data['extra'] = array();
                                                    if (!empty($type['extra'])) {
    		...
    		public function deleteParam($id)
    		{
    			...
    			if ($this->find($id)) {
    				if (M('LiveParamOptions')->where('`param_id`=%d',array($id))->find()) {
    					result('此参数下存在值');
    				}else{
    					if ($this->delete($id)) {
	                                                          result();

<?php
	namespace Home\Live;
	class LiveListContentLive extends \Think\Model
	{
		protected $tableName = 'live_list_content';
		...
		public function updateLiveListContent($nameId,$syncTypeArr)
		{

			$liveListType = D('LiveListType','Live');

			$liveListTypeArr = $liveListType->getLiveListTypeArrUrlForNameID($nameId);

			$content = array();

			foreach ($syncTypeArr as  $value) {
				foreach ($liveListTypeArr as  $k =>$v) {

					if ($value['id'] == $v['id']) {
						$content[] = $v;
						unset($liveListTypeArr[$k]);
						break;
					}
				}
			}
			...
			$liveListTypeArr = $liveListType->getLiveListTypeArrUrlSimpForNameID($nameId);
			$content = array();
			foreach ($syncTypeArr as  $value) {
				foreach ($liveListTypeArr as  $k =>$v) {
			...
			$contentSimpJson = json_encode($content,JSON_UNESCAPED_UNICODE);

			$options = array(
				'name_id'=>$nameId,
				'version'=>time(),
				'content'=>!empty($contentJson)?$contentJson:"[]",
				'content_simp'=>!empty($contentSimpJson)?$contentSimpJson:"[]",
			);

			if ($res = $this->getOneForNameId($nameId)) {
		...
		public function getOneForNameId($NameId)
		{
			return $this->where("name_id = '%s'" ,array($NameId))->find();
		}
